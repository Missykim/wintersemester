...
name=input('Enter what name you would like to look up?')
...
def print_death_age(people_dict):
    ...
    for person_key, person_list in people_dict.items():  
        ...
        print(f'Year of death: {DEATH_YEAR_INDEX} \nAge at time of death:{death_age} \n')
...
def count_genders(people_dict):
    ...
    for person_key, person_list in people_dict.items():
        ...
        if gender == "F":
            count_of_females += 1
    print(f'There are {count_of_males} males.')
    print(f'There are {count_of_females} females.')
...
def print_marriages(marriages_dict, people_dict):
    ...
    #search for wedding year
   
    #search for ages and names 
