...
def make_periodic_table(): 
    ...
    for sub_list in periodic_table:
        #runs through the list 
        symbol = SYMBOL_INDEX[0]
        name = NAME_INDEX[0]
        atomic_mass = ATOMIC_MASS_INDEX[2]
        print(f" Name: {name}\n Atomic mass: {atomic_mass}\n")
    return periodic_table
    
#def make_periodic_table (symbols, name, atomic_mass):
def main():
    
    # Get a chemical formula for a molecule from the user.
    formula=input('What molecule would you like to calculate?')
    # Get the mass of a chemical sample in grams from the user.
    mass= input('What is the molecular mass (in grams) of the chemical')
